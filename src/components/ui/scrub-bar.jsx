...
// Import from local file since we are in the same directory
...
function ScrubBarTrack({ className, children, ...props }) {
    ...
    return (
        <div
            ...
            className={cn(
                "bg-primary/20 relative h-2 w-full grow cursor-pointer touch-none rounded-full transition-none select-none",
                className
            )}
...
function ScrubBarProgress({ className, ...props }) {
    ...
    return (
        <Progress
            ...
            value={progress}
            className={cn("absolute h-full [&>div]:transition-none bg-transparent", className)}
            {...props}
        />
    )
}
...
function ScrubBarThumb({ className, children, ...props }) {
    ...
    return (
        <div
            ...
            className={cn(
                "bg-primary absolute top-1/2 block h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full transition-colors disabled:pointer-events-none disabled:opacity-50 shadow-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
                className
            )}
            style={{ left: `${progress}%` }}
            {...props}
